...
int main() {
	...
	tracker = TrackerMedianFlow::create(); //選擇追蹤方式
	...
	getline(inf, sline); //讀取初始位置的x,y,w,h
	...
	while (true) {
		...
		tracker->update(videoFrame, bbox);//更新追蹤
		rectangle(videoFrame, bbox, Scalar(0, 0, 255), 2, 1);//畫bounding box
		...
		rectangle(videoFrame, Gt, Scalar(255, 0, 0), 2, 1);//畫bounding box
